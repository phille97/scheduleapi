...
def generate_apikey(user):
    session = get_session()
    keypass = hashlib.sha224(str(random.getrandbits(200)).encode('utf-8')).hexdigest()[0:35]
    keyid = str(user.id) + "-" + hashlib.sha224(str(random.getrandbits(200)).encode('utf-8')).hexdigest()[0:35]
